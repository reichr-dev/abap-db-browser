"! <p class="shorttext synchronized">SQL Query Creator</p>
CLASS zcl_dbbr_sql_query_editor DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Returns the last saved query</p>
    "!
  ...
  PROTECTED SECTION.
    ...
    METHODS do_before_dynpro_output REDEFINITION.
    METHODS handle_exit_request     REDEFINITION.

    "! <p class="shorttext synchronized">Checks the modification state of the editor</p>
    METHODS is_modified
      RETURNING
        VALUE(rf_modified) TYPE abap_bool.

    "! <p class="shorttext synchronized">Check Query for errors</p>
    "!
    METHODS check_query
      RETURNING
        VALUE(rf_ok) TYPE abap_bool.

    "! <p class="shorttext synchronized">Saves the current query</p>
    "!
    METHODS save_query.
    "! <p class="shorttext synchronized">Tests the current query string</p>
    METHODS test_query.

    "! <p class="shorttext synchronized">Protect the given lines</p>
...
CLASS zcl_dbbr_sql_query_editor IMPLEMENTATION.
  METHOD constructor.
    ...
    mf_title = |DB Browser - { COND #( WHEN iv_query_name IS INITIAL
                                       THEN |Create Query|
                                       ELSE |Edit Query { iv_query_name }| ) }|.
    mv_query_name = iv_query_name.
    mo_query_f = NEW #( ).

    IF mv_query_name IS NOT INITIAL.
      ms_current_query = mo_query_f->get_query( iv_query_name      = mv_query_name
  ...
  METHOD create_content.
    mo_splitter = NEW zcl_uitb_gui_splitter_cont( iv_elements = 2
    ...
    create_control_toolbar( EXPORTING io_parent  = mo_splitter->get_container( 2 )
                                      it_button  = VALUE #( ( function  = c_functions-toggle_side_bar
    ...
    DATA(lr_f_use_text_based_editor) = CAST abap_bool(
        zcl_uitb_data_cache=>get_instance( zif_dbbr_c_report_id=>user_settings )->get_data_ref(
            zif_dbbr_user_settings_ids=>c_deactvt_highltng_in_cqe ) ).

    mo_editor = NEW zcl_uitb_gui_code_editor( io_parent      = lo_container
*                                              iv_source_type = COND #( WHEN lr_f_use_text_based_editor->* = abap_true THEN 'TEXT' ELSE 'ABAP' )
  ...
  METHOD zif_uitb_gui_command_handler~execute_command.
    CASE io_command->mv_function.
      ...
      WHEN c_functions-format_source.
        mo_editor->get_sel_position( IMPORTING ev_line = DATA(lv_current_line)
  ...
  METHOD do_before_dynpro_output.
    ...
    io_callback->map_fkey_functions( VALUE #( ( fkey            = zif_uitb_c_gui_screen=>c_functions-ctrl_f1
                                                mapped_function = c_functions-focus_on_editor
                                                text            = |{ 'Set focus to Editor' }| )
                                              ( fkey            = zif_uitb_c_gui_screen=>c_functions-shift_f9
                                                mapped_function = c_functions-focus_on_sidebar
                                                text            = |{ 'Set focus to Sidebar' }| )
                                              ...
                                              ( fkey            = zif_uitb_c_gui_screen=>c_functions-shift_f1
                                                mapped_function = c_functions-format_source
                                                text            = |{ TEXT-003 }| )
                                              ( fkey            = zif_uitb_c_gui_screen=>c_functions-f8
                                                mapped_function = c_functions-test_query
                                                text            = |{ TEXT-002 }| )
                                              ( fkey            = zif_uitb_c_gui_screen=>c_functions-ctrl_f5
                                                mapped_function = c_functions-load_entities
                                                text            = |{ TEXT-007 }| )
                                              ( fkey            = zif_uitb_c_gui_screen=>c_functions-f5
                                                mapped_function = c_functions-show_code_completion
                                                text            = |{ TEXT-004 }| )
                                              ( fkey            = zif_uitb_c_gui_screen=>c_functions-f6
                                                mapped_function = c_functions-insert_with_indent
                                                text            = |{ TEXT-008 }| )
  ...
  METHOD handle_exit_request.
    ...
    IF mo_editor IS BOUND AND is_modified( ).
      ...
      DATA(lv_prompt) = zcl_dbbr_appl_util=>popup_to_confirm( iv_title     = 'Warning'
  ...
  METHOD is_modified.
    ...
    IF rf_modified = abap_true.
      RETURN.
    ENDIF.

    DATA(lv_text) = mo_editor->get_text( ).
    DATA(lv_text_upper) = to_upper( lv_text ).
    rf_modified = xsdbool(     lv_text IS NOT INITIAL
                           AND (    lv_text_upper CS 'SELECT'
                                 OR lv_text_upper CS 'WITH' )
                           AND lv_text <> mv_last_saved_content ).
  ENDMETHOD.
  ...
  METHOD check_query.
    ...
    TRY.
        ...
        zcl_dbbr_screen_helper=>show_progress( iv_progress = 1
                                               iv_text     = |{ TEXT-001 }...| ).
      ...
      CATCH zcx_dbbr_sql_query_error INTO DATA(lx_parse_error).
        ...
        IF lx_parse_error->message IS NOT INITIAL.
          lo_protocol->add_error( iv_message     = lx_parse_error->message
  ...
  METHOD save_query.
    IF ms_current_query-query_id IS NOT INITIAL.
      ...
      ENDIF.
    ELSE.
      DATA(lo_save_controller) = NEW zcl_dbbr_save_sql_query_ctrl( is_query_info       = VALUE #(
  ...
  METHOD init_side_bar.
    ...
    mo_side_bar = NEW #( io_parent_container = mo_splitter->get_container( 1 )
                         io_parent_view      = me ).
  ENDMETHOD.
  ...
  METHOD load_entities.
    ...
    init_side_bar( ).
    mo_side_bar->zif_uitb_gui_composite_view~execute_command(
        NEW zcl_uitb_gui_simple_command( iv_function = zif_dbbr_c_sql_query_editor=>fc_load_entity
  ...
  METHOD insert_from_clipboard.
    ...
    IF lt_clipboard_data IS INITIAL.
      RETURN.
    ENDIF.
    mo_editor->get_sel_position( IMPORTING ev_pos = DATA(lv_pos) ).
