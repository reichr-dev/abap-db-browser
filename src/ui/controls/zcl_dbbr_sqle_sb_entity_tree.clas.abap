"! <p class="shorttext synchronized">Entity Tree in SQL Editor Sidebar</p>
CLASS zcl_dbbr_sqle_sb_entity_tree DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized">Loads entities into tree</p>
    "!
  ...
  PRIVATE SECTION.
    ...
    TYPES ty_t_node_type TYPE TABLE OF i.
    ...
    "! <p class="shorttext synchronized">Creates the tree model and control</p>
    "!
    METHODS create_tree.

    "! <p class="shorttext synchronized">Create tree node</p>
    METHODS create_node
      IMPORTING
    ...
    "! <p class="shorttext synchronized">Handler for when children are to be loaded lazily</p>
    "!
    ...
    "! <p class="shorttext synchronized">Handler for requesting a context menu for a node</p>
    "!
    ...
    "! <p class="shorttext synchronized">Handler for when the context menu entry was chosen</p>
    "!
    ...
    "! <p class="shorttext synchronized">Handler for double click on node</p>
    "!
    ...
    "! <p class="shorttext synchronized">Handler for node key press</p>
    "!
    ...
    "! <p class="shorttext synchronized">Handler for ENTER key press on node</p>
    "!
    ...
    "! <p class="shorttext synchronized">Handler for pressed toolbar button</p>
    "!
...
CLASS zcl_dbbr_sqle_sb_entity_tree IMPLEMENTATION.
  ...
  METHOD load_entities.
    ...
    SELECT FROM zsat_i_databaseentity
  ...
  METHOD create_tree.
    ...
    mo_tree = NEW #( ir_parent           = lo_client
                     is_hierarchy_header = VALUE #( heading = 'Object Name'
                                                    width   = 60 )
    ...
    SET HANDLER on_node_context_menu_request FOR lo_events.
    SET HANDLER on_node_context_menu_select FOR lo_events.
    SET HANDLER on_node_double_click FOR lo_events.
    SET HANDLER on_node_key_press FOR lo_events.
*      on_node_enter_key FOR lo_events,
    SET HANDLER on_expand_no_children FOR lo_events.
    SET HANDLER on_tree_drag_multiple FOR lo_events.
    SET HANDLER on_toolbar_button FOR mo_toolbar.

    mo_replace_dnd_behavior = NEW cl_dragdrop( ).
    mo_replace_dnd_behavior->add( flavor     = zcl_uitb_gui_code_editor=>c_dnd_flavor-replace
                                  dragsrc    = abap_true
                                  droptarget = abap_false
                                  effect     = cl_dragdrop=>copy ).
    mo_insert_dnd_behavior = NEW cl_dragdrop( ).
    mo_insert_dnd_behavior->add( flavor     = zcl_uitb_gui_code_editor=>c_dnd_flavor-insert
                                 dragsrc    = abap_true
                                 droptarget = abap_false
                                 effect     = cl_dragdrop=>copy ).
  ENDMETHOD.
  ...
  METHOD on_expand_no_children.
    ...
    IF <ls_user_data> IS NOT ASSIGNED.
      RETURN.
    ENDIF.
  ...
  METHOD on_node_key_press.
    DATA lv_rc TYPE i.
    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA lt_clipboard TYPE TABLE OF char256.
    DATA lo_node TYPE REF TO zcl_uitb_ctm_node.
    DATA lv_clipboard_content TYPE string.
    DATA lt_nodes TYPE zcl_uitb_ctm_node=>ty_t_ctm_node.
      ...
      WHEN cl_tree_model=>key_copy.
        ...
        LOOP AT lt_nodes INTO lo_node.
          ...
          CASE <ls_user_data>-node_type.
            ...
            WHEN OTHERS.
              lv_clipboard_content = |{ lo_node->get_item(
                                            iv_item_name = zcl_uitb_column_tree_model=>c_hierarchy_column )->get_text( ) }{ lv_sep }|.
          ENDCASE.
          lt_clipboard = VALUE #( BASE lt_clipboard ( CONV #( lv_clipboard_content ) ) ).
        ENDLOOP.

        cl_gui_frontend_services=>clipboard_export( IMPORTING data = lt_clipboard
                                                    CHANGING  rc   = lv_rc ).
        IF sy-subrc <> 0.
        ENDIF.
  ...
  METHOD on_node_double_click.
    ...
    CASE <ls_user_data>-node_type.
      ...
      WHEN c_node_type-cds_view OR
        ...
        paste_select_clause( io_node      = lo_node
  ...
  METHOD on_tree_drag_multiple.
    ...
    collect_node_infos( EXPORTING it_node_key              = et_node_key_table
    ...
    ELSE.
      ...
      DATA(lo_dnd_object) = NEW zcl_uitb_gui_editor_dnd_object( iv_text          = lv_content
  ...
  METHOD create_node.
    ...
    DATA(lv_image) = COND #( WHEN iv_image IS INITIAL AND if_folder = abap_false
                             THEN zif_dbbr_c_icon=>no_icon
                             ELSE iv_image ).
    DATA(lv_expanded_image) = COND #( WHEN iv_expanded_image IS INITIAL THEN lv_image ELSE iv_expanded_image ).

    DATA(lt_items) = VALUE treemcitab( ( class     = cl_item_tree_model=>item_class_text
    ...
    TRY.
        ro_node = mo_tree->get_nodes( )->add_node( if_folder            = if_folder
      ...
      CATCH zcx_uitb_tree_error INTO DATA(lx_error).
        lx_error->zif_uitb_exception_message~print( iv_msg_type = 'X' ).
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD load_entity.
    ...
    DATA lv_assoc_node_type TYPE i.
    ...
    CASE ir_user_data->node_type.
      ...
      WHEN c_node_type-table OR
        ...
        zcl_sat_ddic_repo_access=>get_table_field_infos( EXPORTING iv_tablename    = ir_user_data->entity_id
                                                         IMPORTING et_table_fields = lt_fields ).
        fill_view_table_node( io_node        = io_node
                              it_fields      = lt_fields
      ...
      WHEN c_node_type-cds_view.
        TRY.
            DATA(lo_cds_view) = zcl_sat_cds_view_factory=>read_cds_view(
                                    iv_cds_view = to_upper( ir_user_data->entity_id ) ).
  ...
  METHOD fill_cds_node.
    " TODO: parameter IF_EXPAND_NODE is never used (ABAP cleaner)
    ...
    IF if_association = abap_false AND io_cds_view->has_parameters( ).
      DATA(lo_params_node) = create_node( iv_parent_node     = io_node->mv_node_key
                                          if_folder          = abap_true
                                          if_expander        = abap_true
      ...
      LOOP AT lt_params ASSIGNING FIELD-SYMBOL(<ls_param>).
        DATA(lv_sep) = COND #( WHEN sy-tabix < lines( lt_params )
                               THEN |,{ cl_abap_char_utilities=>newline }{ lv_spaces }| ).
        ir_user_data->from_part = ir_user_data->from_part &&
                                 |{ <ls_param>-parametername_raw } = '{ <ls_param>-default_value }'{ lv_sep }|.
        create_node( iv_parent_node     = lo_params_node->mv_node_key
                     iv_image           = |{ icon_parameter_import }|
                     iv_hier1_item_text = |{ <ls_param>-parametername_raw }|
                     iv_handle          = lv_handle
                     is_user_data       = VALUE #( root_key            = lv_root_key
                                                   entity_id           = io_cds_view->get_header( )-entityname
    ...
    DATA(lo_fields_node) = create_node( iv_parent_node     = io_node->mv_node_key
                                        if_folder          = abap_true
                                        if_expander        = abap_true
    ...
    zcl_sat_ddic_repo_access=>get_table_field_infos( EXPORTING iv_tablename    = io_cds_view->get_header( )-entityname
    ...
    LOOP AT io_cds_view->get_columns( ) ASSIGNING FIELD-SYMBOL(<ls_column>).
      ...
      CHECK sy-subrc = 0.
      create_node( iv_parent_node     = lo_fields_node->mv_node_key
                   iv_hier1_item_text = |{ <ls_column>-fieldname_raw }|
                   is_hier2_item      = VALUE #( text  = <ls_field>-fieldtext
    ...
    IF io_cds_view->has_associations( ).
      DATA(lo_assocs_node) = create_node( iv_parent_node     = io_node->mv_node_key
                                          if_folder          = abap_true
                                          if_expander        = abap_true
      ...
      LOOP AT io_cds_view->get_associations( ) ASSIGNING FIELD-SYMBOL(<ls_assoc>).
        create_node( iv_parent_node     = lo_assocs_node->mv_node_key
                     if_folder          = abap_true
                     if_expander        = abap_true
                     iv_hier1_item_text = |{ <ls_assoc>-raw_name }|
                     is_hier2_item      = VALUE #( text  = <ls_assoc>-ddtext
                                                   style = zif_uitb_c_ctm_style=>inverted_gray )
  ...
  METHOD collect_node_infos.
    ...
    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA lt_node_key_range TYPE RANGE OF tm_nodekey.
    DATA lt_node_type TYPE HASHED TABLE OF i WITH UNIQUE KEY table_line.
    ...
    LOOP AT it_node_key INTO DATA(lv_node_key).
      ...
      IF    lr_user_data->node_type = c_node_type-fields
         OR lr_user_data->node_type = c_node_type-parameters.
  ...
  METHOD paste_select_clause.
    " TODO: parameter IO_NODE is never used (ABAP cleaner)

    DATA lt_select TYPE string_table.

    DATA(lr_select) = NEW string( ).

    lt_select = VALUE #( ( |SELECT| )
